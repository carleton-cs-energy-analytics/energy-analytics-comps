...
class PointType:
    ...
    def add_self_to_database(self): # possibly get passed connection to db
        if self.return_type == "enumerated":
            db.execute("""INSERT INTO PointTypes (Name, Units, ReturnType)
    ...
    def get_id_from_database(self):
        db.execute("SELECT * FROM PointTypes WHERE Name = %s;", (self.name))
        return db.fetchone()
...
def get_known_types():
    ...
    """
    known_types = {}
    db.execute("SELECT * FROM PointTypes")
...
def add_point_to_db(point_name, room_id, point_source_id):
    ...
    """
    db.execute("SELECT * FROM PointTypes WHERE Name = %s AND RoomID = %s;", (point_name, room_id))
    ...
    db.execute("""INSERT INTO Points (Name, RoomID, PointTypeID, PointSourceID, Description)
                VALUES (%s, %s, %s, %s, %s);""", (point_name, room_id, point_type.get_id_from_database(),
