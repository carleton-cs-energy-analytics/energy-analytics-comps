...
class SiemensData:
	...
	def find_data_for_room(self, room_name):
		equipment_dictionary = {} #Key = equipment name, value = sorted list of tuples containing (timestamp, data)
		...
		for header in self.data:
			if room_name in header:
				equipment_data = [] # Initialize container for (timestamp, data) tuples

				equipment_name = self.find_equipment_name(header) #Key for dictionary
	...
	def create_datetimes_list(self):
		datetimes = []
		for i in range(len(self.data.columns.values)): # Iterate through all rows
			# Get date and time from particular column
			date = self.data.get_value(i, "<>Date")
			time = self.data.get_value(i, "Time")

			timestamp = date + " " + time
			datetime_object = datetime.strptime(timestamp, "%m/%d/%y %H:%M:%S") #create datetime object from MM/DD/YY xx:xx:xx format
